...
void List::addRandomNodes(int from, int count) {
	int i=from, key=from;
	while (i<count+from) if(addNode(key++)) i++;

	/*
	srand(time(NULL));
	int key=(rand() % 100*count), i=0;
	while (i<count) if(addNode(key + tail->getKey())) { 
	i++;
	key=(rand() % 100*count);
	}*/
}
